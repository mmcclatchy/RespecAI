...
"""
    ...
    def test_full_workflow_integration(
        self, state_manager: InMemoryStateManager, completion_report_tools: PlanCompletionReportTools
    ) -> None:
        ...
        store_result = completion_report_tools.store_completion_report(completion_report, loop_id)
        ...
        retrieved_report = completion_report_tools.get_completion_report_data(loop_id)
        ...
        markdown_result = completion_report_tools.get_completion_report_markdown(loop_id)
        ...
        update_result = completion_report_tools.update_completion_report(updated_report, loop_id)
        assert 'Updated Integration Test Report' in update_result.message

        # Verify update
        updated_retrieved = completion_report_tools.get_completion_report_data(loop_id)
        assert updated_retrieved.report_title == 'Updated Integration Test Report'
        assert updated_retrieved.final_plan_score == '95'

        # Step 6: List reports
        list_result = completion_report_tools.list_completion_reports()
        ...
        delete_result = completion_report_tools.delete_completion_report(loop_id)
        assert 'Updated Integration Test Report' in delete_result.message

        # Verify deletion
        list_result_after_delete = completion_report_tools.list_completion_reports()
    ...
    def test_multiple_reports_workflow(
        ...
        for i, loop_state in enumerate(loop_states):
            ...
            result = completion_report_tools.store_completion_report(completion_report, loop_state.id)
        ...
        # Test count limit
        limited_list = completion_report_tools.list_completion_reports(count=2)
        assert 'Found 2 completion reports' in limited_list.message

        # Verify each report can be retrieved individually
        for i, loop_state in enumerate(loop_states):
            retrieved = completion_report_tools.get_completion_report_data(loop_state.id)
    ...
    def test_error_scenarios_integration(
        ...
        with pytest.raises(Exception):  # Should raise ResourceError via LoopNotFoundError
            completion_report_tools.store_completion_report(completion_report, non_existent_loop_id)
        ...
        with pytest.raises(Exception):  # Should raise ResourceError
            completion_report_tools.get_completion_report_data(loop_state.id)
